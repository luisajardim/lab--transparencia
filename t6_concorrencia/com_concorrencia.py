...
@contextmanager
def distributed_lock(r: redis.Redis, resource: str, ttl: int = 5, retry_delay: float = 0.01, max_retries: int = 500):
    """
    ...
    if not acquired:
        raise RuntimeError(f"Recurso '{resource}' em uso — timeout apos {max_retries} tentativas")
    try:
        yield
    finally:
